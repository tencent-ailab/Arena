...
from arena.utils.vizdoom.player_vs_f1 import PlayerHostConfig, PlayerJoinConfig, PlayerConfig
from arena.utils.vizdoom.player_vs_f1 import player_setup, player_host_setup, player_join_setup
    ...
    def act(self, action):
        ...
        if action in [self.all_actions[0], self.all_actions[2], self.all_actions[3]]:
            if self.repeat_action(action) % 16 == 0:
                ...
                # print('repeat_action')
        if action in [self.all_actions[0], self.all_actions[2], self.all_actions[3], self.all_actions[4],
                      self.all_actions[5]]:
            if is_attacking:
...
class VizdoomMPEnv(gym.Env):
    ...
    def __init__(self, config_path,
        ...
        self.mode = mode
        # join cfg
        self.join_cfg = PlayerJoinConfig()
        # player cfg
        self.players_cfg = []
...
class VizdoomVecRwd(Wrapper):
    ...
    def step(self, action):
        ...
        if done:
            ...
            #     Frag = np.append(Frag, [self.envs[j].unwrapped._game_vars['FRAGCOUNT']])
            ...
            #
            # rwds = [(outcome[0],) + rwd_p1[1:], (outcome[1],) + rwd_p2[1:], (outcome[2],) + rwd_p3[1:], (outcome[3],
            #                                                                                              ) + rwd_p4[1:],
            #         (outcome[4],) + rwd_p5[1:], (outcome[5],) + rwd_p6[1:], (outcome[6],) + rwd_p7[1:],
            #         (outcome[7],) + rwd_p8[1:]]
